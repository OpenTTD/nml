...
def parse_randomswitch_type(random_switch):
    ...
    if type_str not in random_types[feature_val]:
        raise generic.ScriptError("Invalid combination for random_switch feature %d and type '%s'. " % (feature_val, type_str), type_pos)
...
def parse_randomswitch(random_switch):
    ...
    for choice in random_switch.choices:
        ...
        result, comment = action2var.parse_result(choice.result, action_list, act6, offset, random_action2, None, res_prob)
        offset += res_prob * 2
