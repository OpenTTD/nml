...
varact2vars_vehicles = {
    ...
    "num_vehs_in_consist": {
        ...
        "start": 16,
        "size": 8,
        "value_function": value_add_constant(1),
    },  # Zero-based, add 1 to make sane
...
varact2vars60x_vehicles = {
    ...
    "other_veh_curv_info": {
        ...
        "value_function": value_sign_extend,
    },
...
def platform_info_param(name, args, pos, info):
    ...
    if is_middle and args[0].value == 2:
        raise generic.ScriptError(
            "Invalid argument for '{}'(), PLATFORM_SAME_DIRECTION is not supported here.".format(name), pos
        )
...
varact2vars60x_stations = {
    **varact2vars60x_base_stations,
    ...
    "platform_length": {
        ...
        "start": 16,
        "size": 4,
        "param_function": platform_info_param,
        "value_function": platform_info_fix_var,
    },
    "platform_count": {
        "var": 0x00,
        "start": 20,
    ...
    "platform_position_from_middle": {
        "var": 0x00,
        "start": 0,
        "size": 4,
        "param_function": platform_info_param,
        "middle": True,  # 'middle' is used by platform_info_param
        "value_function": lambda var, info: value_sign_extend(platform_info_fix_var(var, info), info),
    },
    "platform_number_from_middle": {
        "var": 0x00,
        "start": 4,
        "size": 4,
        "param_function": platform_info_param,
...
varact2vars_houses = {
    ...
    "same_class_count_town": {
        ...
        "start": 16,
        "size": 8,
        "value_function": house_same_class,
    },  # 'var' is unused
        ...
        for i, offs in enumerate(offsets[:]):
            if isinstance(offs, expression.ConstantNumeric):
                generic.check_range(
                    ...
                    -128,
                    127,
                    "{}-parameter {:d} '{}offset'".format(name, i + 1, "x" if i == 0 else "y"),
                    pos,
                )
...
varact2vars60x_houses = {
    ...
    "nearest_house_matching_criterion": {
        ...
        "start": 0,
        "size": 8,
        "param_function": nearest_house_matching_criterion,
    },
...
varact2vars_industries = {
    ...
    "transported_last_month_pct_1": {
        ...
        "replaced_by": "transported_last_month_pct",
    },
...
def industry_count(name, args, pos, info):
    if len(args) < 1 or len(args) > 2:
        raise generic.ScriptError(
            "'{}'() requires between 1 and 2 argument(s), encountered {:d}".format(name, len(args)), pos
        )
...
def industry_layout_count(name, args, pos, info):
    if len(args) < 2 or len(args) > 3:
        raise generic.ScriptError(
            "'{}'() requires between 2 and 3 argument(s), encountered {:d}".format(name, len(args)), pos
        )
...
varact2vars_towns = {
    ...
    "cities_enabled": {
        ...
        "start": 1,
        "size": 1,
        "value_function": lambda var, info: expression.Not(var, var.pos),
    },
...
varact2vars_roadstop = {
    **varact2vars_base_stations,
    ...
    "company_colour1": {
        ...
        "start": 24,
        "size": 4,
    },  # COLOUR_XXX. See https://newgrf-specs.tt-wiki.net/wiki/NML:List_of_default_colour_translation_palettes#Company_colour_helper_functions
...
varact2vars60x_roadstop = {
    **varact2vars60x_base_stations,
    ...
    "nearby_tile_is_drive_through": {
        "var": 0x68,
        "start": 12,
        "size": 4,
        "param_function": signed_tile_offset,
        "value_function": lambda var, info: nmlop.CMP_GE(var, 4),
    },
