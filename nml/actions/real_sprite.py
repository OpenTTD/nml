    ...
    for sprite in sprite_list:
        if isinstance(sprite, nml.ast.RealSprite):
            ...
            if sprite.name not in sprite_template_map:
                raise ScriptError("Encountered unknown template identifier: " + name)
            template = sprite_template_map[sprite.name]
            if len(sprite.param_list) != len(template.param_list):
                raise ScriptError("Incorrect number of template arguments. Expected " + str(len(template.param_list)) + ", got " + str(len(sprite.param_list)))
            param_dict = {}
            try:
                i = 0
                ...
                raise ScriptError("Template parameters should be compile-time constants")
