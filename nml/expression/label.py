...
class Label(ConstantNumeric):  # lgtm[py/missing-equals]
    """
    A 4-byte string (e.g. grfids, cargo/railtype labels) that's equivalent to a dword.

    This allows us to produce more-readable NFO when the value isn't modified, without
     upsetting code that expects a ConstantNumeric.

    (lgtm tag: ConstantNumeric's __eq__ ignores string_value, which is intended)
    """
